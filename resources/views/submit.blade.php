    ...
    <div class="flex items-center justify-center h-screen">
        <form action="{{route('link.store')}}" method="post"
            ...
            <div class="md:flex md:items-center mb-6">
                ...
                <div class="md:w-2/3">
                    <input id="url" value="{{ old('url') }}" name="url"
                           class="appearance-none border-2 @error('url') border-red-400 @else border-gray-200 @enderror rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-teal-500"
                           type="text">
                    @error('url')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
